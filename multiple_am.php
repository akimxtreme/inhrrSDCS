...
function analisis($dato,$dato2){
	...
	div("inicio");
		...
		$sql = "SELECT * FROM $dato2 WHERE usuario='$dato'";
...
//analisis($user,'tmp_analisis_muestras');
	if(($codigo!="Seleccione...")){
		...
		$sql = "SELECT * FROM tmp_analisis_muestras WHERE analisis_muestras='$campo1' AND usuario='$user'";
		$seleccion=mysql_query($sql,$conexion);
		$cont=mysql_num_rows($seleccion);
			if($cont>=1){
				analisis($user,'tmp_analisis_muestras');
			}else{
				$sql = "INSERT INTO tmp_analisis_muestras (usuario, analisis_muestras) VALUE ('$user', '$campo1')";
	...
	}else { analisis($user,'tmp_analisis_muestras');}
